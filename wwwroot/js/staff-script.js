...
async function confirmOrder() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            let text;
            try { text = (await res.json()).message ?? JSON.stringify(await res.text()); }
...
// Giả lập dữ liệu có sẵn trong hệ thống (Thực tế sẽ lấy từ Database)
const mockOrders = [
    { sdt: '0912345678', type: 'Đơn hàng', code: 'DH001', status: 'Đang giao', detail: 'Phở bò, Bánh mì' },
    { sdt: '0912345678', type: 'Đặt bàn', code: 'DB_99', status: 'Đã xác nhận', detail: '4 người, 19:00 hôm nay' }
];
...
function findOrder() {
    ...
    const results = mockOrders.filter(item => item.sdt === phoneInput);
            ...
            html += `
                <div style="background:#f0f7ff; padding:10px; border-radius:8px; margin-bottom:10px; border-left:4px solid #007bff;">
                    <div style="display:flex; justify-content:space-between;">
                        <span style="font-weight:bold; color:#007bff;">[${item.type}] #${item.code}</span>
                        <span style="font-size:12px; background:#fff; padding:2px 8px; border-radius:10px; border:1px solid #ddd;">${item.status}</span>
                    </div>
                    <p style="font-size:13px; margin-top:5px; color:#555;">${item.detail}</p>
                </div>`;
        });
        resultContainer.innerHTML = html;
    } else {
        resultContainer.innerHTML = `<p style="color:red; text-align:center;">Không tìm thấy thông tin cho số điện thoại này!</p>`;
    }
}
...
function openModal(action) {
    ...
    switch (action) {
            ...
            formHtml = `
                <div class="booking-form" style="display:flex; flex-direction:column; gap:15px; text-align:left;">
                    <div class="form-group">
                        ...
                        <input type="text" placeholder="Nhập tên của bạn" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:5px;">
                    </div>
                    <div class="form-group">
                        <label style="font-weight:bold; font-size:14px;">Số điện thoại:</label>
                        <input type="tel" placeholder="Ví dụ: 0912xxxxxx" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:5px;">
                    </div>
                    <div style="display:flex; gap:10px;">
                        ...
                        <div style="flex:1;">
                            <label style="font-weight:bold; font-size:14px;">Giờ đến:</label>
                            <input type="time" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:5px;">
                        </div>
                    </div>
                    <div class="form-group">
                        <label style="font-weight:bold; font-size:14px;">Số lượng người:</label>
                        <input type="number" min="1" value="1" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:5px;">
                    </div>
                    <button style="padding:12px; background-color:#e44d26; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">XÁC NHẬN ĐẶT BÀN</button>
                </div>`;
            break;
        ...
        // Trong hàm openModal, sửa lại case 'tra cứu':
        case 'tra cứu':
